...
async function startServer() {
  ...
  app.post("/api/submit-lead", async (req, res) => {
    ...
    const messageText = `
\u{1F31F} Novo Lead - Traffio Odonto \u{1F31F}

\u{1F464} Nome: ${name}
\u{1F4E7} E-mail: ${email}
\u{1F3E5} Cl\xEDnica: ${clinic}
\u{1F4F1} WhatsApp: ${whatsapp}
\u{1F4B0} Or\xE7amento Mensal: ${budget}
`.trim();
    const messageHtml = `
      <h2>\u{1F31F} Novo Lead - Traffio Odonto \u{1F31F}</h2>
      <p><strong>\u{1F464} Nome:</strong> ${name}</p>
      <p><strong>\u{1F4E7} E-mail:</strong> ${email}</p>
      <p><strong>\u{1F3E5} Cl\xEDnica:</strong> ${clinic}</p>
      <p><strong>\u{1F4F1} WhatsApp:</strong> ${whatsapp}</p>
      <p><strong>\u{1F4B0} Or\xE7amento Mensal:</strong> ${budget}</p>
    `;
    ...
    if (tgToken && tgChatId) {
      try {
        ...
        console.error("Erro de conex\xE3o com Telegram:", e);
      }
    } else {
      console.log("Credenciais do Telegram n\xE3o configuradas.");
    }
    ...
    if (smtpHost && smtpUser && smtpPass && emailTo) {
      ...
      console.log("Credenciais de E-mail n\xE3o configuradas.");
    }
